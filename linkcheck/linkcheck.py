...
def fetch_robots_txt(base_url, headers):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if urls:
                print(f"[{Colors.GREEN}SUC{Colors.RESET}] Fetching robots.txt... {len(urls)} URLs found")
                return urls
            else:
                print(f"[{Colors.ORANGE}SKIP{Colors.RESET}] Fetching robots.txt... No URLs found")
                return []
        else:
            print(f"[{Colors.ORANGE}SKIP{Colors.RESET}] Fetching robots.txt... Not found")
            return []
    except requests.RequestException:
        spinner.stop()
        print(f"[{Colors.ORANGE}SKIP{Colors.RESET}] Fetching robots.txt... Failed")
        return []

def crawl_and_check_links(base_url, max_depth=3):
    headers = {
                ...
                if "facebook.com" in full_url and not skip_facebook:
                    try:
                        ...
                        if fb_response.status_code == 400:
                            skip_facebook = True
                            print(f"[{Colors.ORANGE}SKIP{Colors.RESET}] Facebook links returning 400 errors, skipping future checks")
                    except requests.RequestException:
                        pass
    ...
    print(f"[{Colors.CYAN}INF{Colors.RESET}] Crawling target site...\n")
...
def save_json_output(output_file, base_url, broken_links, stats, elapsed):
    ...
    try:
        ...
        spinner.stop()
        print(f"[{Colors.GREEN}SUC{Colors.RESET}] Saving JSON output... Saved to {output_file}")
        return True
    except Exception as e:
        spinner.stop()
        print(f"[{Colors.RED}ERR{Colors.RESET}] Saving JSON output... {e}")
        return False

def print_results(broken_links, stats):
    print(f"[{Colors.GREEN}SUC{Colors.RESET}] Crawl completed")
    ...
    if stats['broken'] > 0:
        print(f"[{Colors.RED}FAIL{Colors.RESET}] Broken links found: {stats['broken']}\n")
        ...
        print(f"[{Colors.GREEN}SUC{Colors.RESET}] No broken links found")
...
def main():
    ...
    parser.add_argument("-u", required=True, metavar="https://example.com",
    ...
    if not args.u.startswith(("http://", "https://")):
        print(f"[{Colors.RED}ERR{Colors.RESET}] URL must start with http:// or https://")
        sys.exit(1)

    print_banner()
    START_TIME = time.time()

    print(f"[{Colors.CYAN}INF{Colors.RESET}] Target: {args.u}")
    print(f"[{Colors.CYAN}INF{Colors.RESET}] Max depth: {args.d}\n")

    broken_links, stats = crawl_and_check_links(args.u, args.d)
    ...
    if args.output:
        save_json_output(args.output, args.u, broken_links, stats, elapsed)
